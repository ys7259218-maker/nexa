...
- `npm run check` is green at the current PR head (553 unit tests passing — `main` @ `cd07b812` had 547 (PR #185's six E.164 contract tests); PR #184's head was 541; this change adds the six global-search contract tests — enforced by `npm run check:documented-count` against the runner total at the same head; `npm audit` 0 vulnerabilities). Operational gates live in `docs/OPERATIONS_RUNBOOK.md`; go-live steps live in `docs/GO_LIVE.md`.
