...
- `npm run check` is green at this PR's head (541 unit tests passing, enforced by `npm run check:documented-count` against the runner total at the same head; `npm audit` 0 vulnerabilities). Operational gates live in `docs/OPERATIONS_RUNBOOK.md`; go-live steps live in `docs/GO_LIVE.md`.
