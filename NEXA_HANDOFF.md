...
- **Production readiness boundary:** no dedicated production Supabase project is proven to exist. The repo documents staging `nexa-beryl-gamma` (`vbizuxxgjlwqotuegskq`) as the linked project, and `docs/GO_LIVE.md` still lists the production Supabase project as not created. Outbound remains `WHATSAPP_OUTBOUND_ENABLED=false`; production/provider/provider-configuration actions remain owner-gated (see `docs/OPERATIONS_RUNBOOK.md` release gates).
