...
- Conversation safety: rollout-gated human takeover immediately stops AI drafts for one conversation; exact whole-message opt-out keywords create durable, audited, non-clearable opt-out state without logging customer content (opt-out recording and draft-blocking run unconditionally as a compliance baseline, independent of the rollout flag)
...
- Owner-scoped conversation inbox at `/conversations`: newest chats, masked customer identifiers, message history, draft-blocked status, rollout-gated human takeover/opt-out state, and loading/empty/error states; sidebar navigation is wired and the route is protected by both proxy and server auth
