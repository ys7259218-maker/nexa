...
export default function WorkspaceKillSwitch({ state }: { state: WorkspaceSafetyState }) {
  ...
  return <Card className="flex items-center justify-between gap-6 border-amber-900/50">
    <div><p className="text-sm text-zinc-500">Workspace safety</p><h2 className="text-xl font-semibold">{state.automationPaused ? "Automation paused" : "Automation permitted"}</h2><p className="mt-1 text-sm text-zinc-400">{state.name} · {canManage ? "Owner/Admin control" : "Read-only"}</p>{message ? <p role="alert" className="mt-2 text-sm text-red-300">{message}</p> : null}</div>
    <Button variant={state.automationPaused ? "secondary" : "danger"} disabled={!canManage || saving} onClick={toggle}>{state.automationPaused ? "Resume automation" : "Pause all automation"}</Button>
  </Card>;
}
