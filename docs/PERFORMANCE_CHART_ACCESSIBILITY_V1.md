...
- Recharts' default accessibility layer is disabled and the decorative SVG has
  `tabIndex={-1}` so keyboard users do not tab into an aria-hidden chart.
...
The chart-focus contract also prevents the default Recharts keyboard target
from returning. Authenticated screen-reader testing remains a separate gate;
public auth smoke tests do not exercise this chart with real account data.
