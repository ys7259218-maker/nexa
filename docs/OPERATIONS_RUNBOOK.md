...
- `NEXT_PUBLIC_SUPABASE_URL` is not the exact staging Supabase project hostname `vbizuxxgjlwqotuegskq.supabase.co`; production must target the reviewed production Supabase project, which does not exist yet (see `docs/GO_LIVE.md`), so production builds remain blocked until one is provisioned and approved.
