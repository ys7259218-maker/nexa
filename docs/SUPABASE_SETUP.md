...
Migration `20260829072333_conversation_safety_controls.sql` adds conversation-level human takeover and durable WhatsApp keyword opt-out state. Human takeover is writable only through an authenticated Owner/Admin/Operator RPC; opt-out is writable only through the server-only service-role RPC. Both paths are serialized, audited without customer content, and protected from direct Data API updates. Keep `CONVERSATION_SAFETY_ENABLED=false` until a dedicated migrated project proves the role matrix, cross-workspace rejection, viewer denial, exact keyword behavior, idempotent opt-out, and fail-closed runtime behavior. The migration does not enable outbound sending and does not provide a browser path to clear customer opt-out.
