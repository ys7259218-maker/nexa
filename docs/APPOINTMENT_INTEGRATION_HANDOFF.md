...
- Head commit: `467f462` "test(integration): read-only booking-schema reconcile against live staging".
...
- `npm run test:integration:appointments` (2/2) and `npm run test:integration:booking-reconcile` (2/2) pass when run with local `.env.local` staging credentials injected; both report **skipped** in CI (no credentials), so a green CI is not staging evidence.
...
1. **Server-only booking adapter run against real staging DB** (the only remaining step; blocked): requires the proposal-only ledger schema to first be applied to staging via a valid staging management token with DB-execute rights (the local `SUPABASE_ACCESS_TOKEN` is revoked — 401 — and `SUPABASE_SERVICE_ROLE_KEY` is empty in Vercel prod/preview), or owner-applied DDL from the dashboard. Then exercise claim/complete/release paths against `appointment_booking_approvals` + `appointment_booking_attempts` with dedicated synthetic workspace/review/messages and clean up. The sandbox E2E flow (`lib/booking/appointmentSandboxFlow.test.ts`) proves the adapter contract without this.
...
- This branch is code-only and unmerged; deleting the branch or reverting `467f462` (and prior head commits) fully reverts this milestone.
