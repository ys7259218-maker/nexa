...
> - **Current tree (this PR, based on `main` @ `cd07b812`):** `npm run check` is green at its head with **553** unit tests — the six global-search contract tests added in this PR. (Historical baselines: PR #184's exact reviewed head `35b4f873` had 541 unit tests; PR #185's head `7adddfc` had 547, adding the six E.164 recipient-validation contract tests; PR #185 was squash-merged into `main` as `cd07b812`. Rebaseline of the older record: "main @ `cf70cdc`, 310+"; the 539 count predated PR #184's two added contract tests.) The stated test total is enforced by `npm run check:documented-count` against the runner's structured total at the same head. `npm audit` shows 0 vulnerabilities.
