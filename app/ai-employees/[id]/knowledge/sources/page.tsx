...
import { isValidKnowledgeSourceId, listKnowledgeSources, type KnowledgeSource } from "@/lib/knowledgeSources";
...
export default async function KnowledgeSourcesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!enabled) error = "Knowledge Source Registry v1 is safely disabled until its migration and dedicated role/RLS checks pass.";
    ...
    if (!supabase) error = "The source registry is unavailable because the app data connection is not configured.";
      ...
      if (employeeResult.error || !employeeResult.data) error = "This AI Employee could not be found or does not belong to your workspace.";
        ...
        const sourceResult = await listKnowledgeSources(supabase, id);
        if (sourceResult.error) error = sourceResult.error;
        else sources = sourceResult.data;
      }
    }
  }
  ...
  return <AppLayout><div className="space-y-8">
    ...
    {error ? <Card className="space-y-3"><h2 className="text-xl font-semibold text-amber-300">Source registry unavailable</h2><p className="text-zinc-400">{error}</p></Card> : employee ? <KnowledgeSourceRegistry employeeId={employee.id} initialSources={sources} /> : null}
  </div></AppLayout>;
}
