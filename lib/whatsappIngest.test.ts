...
// --- Draft-Assist Inbound-Only V1 ---

function seedTestingWorkspace(store: FakeSupabase): void {
  store.tables["workspaces"] = [{ id: "workspace-1", automation_paused: false }];
  store.tables["whatsapp_channels"] = [
    { id: "chan-1", user_id: "owner-1", workspace_id: "workspace-1", ai_employee_id: "emp-1", phone_number_id: "phone-123", display_name: "Main" },
  ];
  store.tables["ai_employees"] = [
    {
      ...
      greeting_message: "Hello! Here to help.",
      knowledge_notes: "",
      status: "Testing",
      lifecycle_status: "Testing",
      automation_paused: false,
...
function seedUnassignedTestingChannel(store: FakeSupabase): void {
  store.tables["workspaces"] = [{ id: "workspace-1", automation_paused: false }];
  store.tables["whatsapp_channels"] = [
    { id: "chan-1", user_id: "owner-1", workspace_id: "workspace-1", ai_employee_id: null, phone_number_id: "phone-123", display_name: "Main" },
  ];
  store.tables["ai_employees"] = [];
}

function seedCrossWorkspaceTestingEmployee(store: FakeSupabase): void {
  store.tables["workspaces"] = [
    { id: "workspace-1", automation_paused: false },
    { id: "workspace-2", automation_paused: false },
  ];
  store.tables["whatsapp_channels"] = [
    { id: "chan-1", user_id: "owner-1", workspace_id: "workspace-1", ai_employee_id: "emp-1", phone_number_id: "phone-123", display_name: "Main" },
  ];
...
function setDraftAssistEnv(values: Record<string, string | undefined>): Record<string, string | undefined> {
...
function draftBlockedOutbounds(store: FakeSupabase): Row[] {
  return (store.tables["messages"] ?? []).filter(
    (row) => row.direction === "outbound" && row.status === "draft_blocked",
  );
}

test("draft-assist: Testing employee + flag true + assigned channel + unpaused workspace stores exactly one draft_blocked reply", async () => {
  const env = setDraftAssistEnv({
  ...
  try {
    const store = new FakeSupabase();
    seedTestingWorkspace(store);

    const summary = await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-1" }),
    ]);

    assert.deepEqual(summary, { accepted: 1, duplicates: 0, skipped: 0, failed: 0 });
    assert.equal(store.tables["conversations"]?.[0]?.ai_employee_id, "emp-1");
    const drafts = draftBlockedOutbounds(store);
    assert.equal(drafts.length, 1);
    assert.equal(drafts[0]?.user_id, "owner-1");
    assert.equal(drafts[0]?.workspace_id, "workspace-1");
    assert.equal(drafts[0]?.direction, "outbound");
    assert.ok(String(drafts[0]?.body).includes("Test Corp"));
  } finally {
    restoreEnv(env);
  }
});

test("draft-assist: flag false or missing creates no draft for a Testing employee", async () => {
  const missing = setDraftAssistEnv({
  ...
  try {
    ...
    const summary = await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-missing-1" }),
    ]);

    assert.deepEqual(summary, { accepted: 1, duplicates: 0, skipped: 0, failed: 0 });
    assert.equal(draftBlockedOutbounds(store).length, 0);
    assert.equal((store.tables["messages"] ?? []).filter((row) => row.direction === "inbound").length, 1);
  } finally {
    restoreEnv(missing);
  }

  const disabled = setDraftAssistEnv({
  ...
  try {
    ...
    await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-false-1" }),
    ]);
...
test("draft-assist: Draft, Paused, and Archived employees never create drafts even with flag true", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
    CONVERSATION_SAFETY_ENABLED: "false",
  ...
  try {
    for (const lifecycle_status of ["Draft", "Paused", "Archived"] as const) {
      const store = new FakeSupabase();
      store.tables["workspaces"] = [{ id: "workspace-1", automation_paused: false }];
      store.tables["whatsapp_channels"] = [
        { id: "chan-1", user_id: "owner-1", workspace_id: "workspace-1", ai_employee_id: "emp-1", phone_number_id: "phone-123", display_name: "Main" },
      ...
      await processWhatsAppEvents(asClient(store), provider, [
        makeTextEvent({ eventId: `wamid.draft-assist-${lifecycle_status.toLowerCase()}-1` }),
      ]);

      assert.equal(draftBlockedOutbounds(store).length, 0, `${lifecycle_status} employee must not draft`);
      assert.equal((store.tables["messages"] ?? []).filter((row) => row.direction === "inbound").length, 1);
    }
  } finally {
    restoreEnv(env);
...
test("draft-assist: human takeover blocks drafts for a Testing employee", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
  ...
  try {
    ...
    const summary = await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-human-1" }),
    ]);
...
test("draft-assist: customer opt-out blocks drafts for a Testing employee", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
    CONVERSATION_SAFETY_ENABLED: "true",
  ...
  try {
    ...
    await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-optout-1", body: "STOP!" }),
    ]);
...
test("draft-assist: workspace pause fails closed and stores no draft", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
    CONVERSATION_SAFETY_ENABLED: "false",
  ...
  try {
    ...
    await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-ws-paused-1" }),
    ]);
...
test("draft-assist: unassigned channel and cross-workspace employees fail closed", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
    CONVERSATION_SAFETY_ENABLED: "false",
  ...
  try {
    const unassigned = new FakeSupabase();
    seedUnassignedTestingChannel(unassigned);

    await processWhatsAppEvents(asClient(unassigned), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-unassigned-1" }),
    ]);

    assert.equal(draftBlockedOutbounds(unassigned).length, 0);
    assert.equal(unassigned.tables["conversations"]?.[0]?.ai_employee_id, null);
    assert.equal((unassigned.tables["messages"] ?? []).filter((row) => row.direction === "inbound").length, 1);

    const crossWorkspace = new FakeSupabase();
    seedCrossWorkspaceTestingEmployee(crossWorkspace);

    await processWhatsAppEvents(asClient(crossWorkspace), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-cross-ws-1" }),
    ]);

    assert.equal(draftBlockedOutbounds(crossWorkspace).length, 0);
    assert.equal((crossWorkspace.tables["messages"] ?? []).filter((row) => row.direction === "inbound").length, 1);
    assert.equal(crossWorkspace.tables["messages"]?.[0]?.workspace_id, "workspace-1");
  } finally {
    restoreEnv(env);
  }
});

test("draft-assist: duplicate inbound events never create duplicate drafts", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
    CONVERSATION_SAFETY_ENABLED: "false",
  ...
  try {
    ...
    const first = await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-dup-1" }),
    ]);
    assert.deepEqual(first, { accepted: 1, duplicates: 0, skipped: 0, failed: 0 });
    assert.equal(draftBlockedOutbounds(store).length, 1);

    const second = await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-dup-1" }),
    ]);
    assert.deepEqual(second, { accepted: 0, duplicates: 1, skipped: 0, failed: 0 });
    assert.equal(draftBlockedOutbounds(store).length, 1);
...
test("draft-assist: no outbound message is ever sent through the inbound path", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
    WORKSPACE_SAFETY_ENABLED: "true",
    CONVERSATION_SAFETY_ENABLED: "false",
  ...
  try {
    ...
    await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-no-send-1" }),
    ]);

    const outbound = draftBlockedOutbounds(store);
    assert.equal(outbound.length, 1);
    assert.equal(outbound[0]?.status, "draft_blocked");
    assert.equal(outbound[0]?.wa_message_id, null);
    assert.equal(
      (store.tables["messages"] ?? []).filter((row) => row.direction === "outbound" && row.status !== "draft_blocked").length,
      0,
    );
    // sendTextMessage / sendTemplateMessage / the Meta transport are never
    // reached: the inbound path only stores a draft through storeOutboundDraft.
  } finally {
    restoreEnv(env);
  }
});

test("draft-assist: WHATSAPP_OUTBOUND_ENABLED=false keeps sends blocked even with flag true", async () => {
  const env = setDraftAssistEnv({
    INBOUND_DRAFT_ASSIST_ENABLED: "true",
  ...
  try {
    ...
    await processWhatsAppEvents(asClient(store), provider, [
      makeTextEvent({ eventId: "wamid.draft-assist-outbound-off-1" }),
    ]);

    assert.equal(draftBlockedOutbounds(store).length, 1);
    assert.equal(
      (store.tables["messages"] ?? []).some((row) => row.direction === "outbound" && row.status === "sent"),
      false,
    );
  } finally {
    restoreEnv(env);
