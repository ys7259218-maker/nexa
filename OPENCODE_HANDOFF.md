...
8. Done: Meta delivered/read/failed receipts deduplicate through the ledger and update only the owner&apos;s matching outbound message without status regression. Next consider moving webhook processing from inline request handling to a queue/worker using the same processor boundary (schema unchanged), and add an admin-triggered retry endpoint wrapping `retryPendingWebhookEvents`.
