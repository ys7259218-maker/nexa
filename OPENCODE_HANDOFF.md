...
5. Build a real conversation inbox UI reading `conversations`/`messages` (owner read policies already exist), and wire delivery receipts into message statuses once outbound ships.
...
7. Replace the mock AI provider with a real provider behind the same interface in a server-only module; keep keys out of the repo and never log message bodies or customer numbers.
8. Consider moving webhook processing from inline request handling to a queue/worker using the same processor boundary (schema unchanged), and add an admin-triggered retry endpoint wrapping `retryPendingWebhookEvents`.
